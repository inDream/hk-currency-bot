...
class Stat {
  ...
  getRecent() {
    let sql = `(SELECT * FROM stats ORDER BY count DESC LIMIT 5)
    UNION (SELECT * FROM stats ORDER BY "updatedAt" DESC LIMIT 5)
  ...
  dialogRecent(dialog) {
    ...
    return this.getRecent()
      .then(items => {
        ...
        keys.push([
          {text: 'Others', callback_data: 'Others0'},
          {text: 'Others (Reverse)', callback_data: 'Others1'}
        ]);
        let options = {
