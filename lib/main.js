...
class Main {
  ...
  listCurrency(code) {
    let self = this;
    if (config.availCurrencies.indexOf(code.toUpperCase()) > 0) {
      return self._listCurrency_sub(code);
    } else if (code.toUpperCase() === 'ALL') {
      let res = '';
      config.availCurrencies.forEach(code => {
        res += self._listCurrency_sub(code);
      });
      return res;
    }
  }

  _listCurrency_sub(code) {
    let self = this;
    ...
    fields.forEach(f => {
      let bank = price[f + 'b'];
      if (bank) {
        res += names[i] + ': ' + price[f] + ' ' + self.banks[bank] + '\n';
      }
      i++;
    });
    return res;
