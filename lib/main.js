...
class Main {
  ...
  inlineBanks() {
    ...
    let temp = [];
    let i = 0;
    Object.keys(this.banks).forEach(b => {
      temp.push({text: this.banks[b], callback_data: 'Bank' + b});
      i++;
      if (i % 4 === 0) {
        keys.push(temp);
        temp = [];
  ...
  inlineOthers(reverse, bank) {
    ...
    config.availCurrencies.forEach(c => {
      if (reverse) {
        ...
        temp.push({text: textRev, callback_data: `${c} ${bank} 1`});
      } else {
        let text = c.toUpperCase() + '/HKD';
        temp.push({text: text, callback_data: `${c} ${bank} 0`});
      }
      i++;
      if (i % 4 === 0) {
        keys.push(temp);
